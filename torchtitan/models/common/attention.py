...
#   B = singleton kernel batch, T = packed tokens, L = sequence length,
#   D = model dimension,
...
class FlexAttention(Module):
    ...
    def compiled_flex_attn(
        ...
        *,
        ...
        is (B, H, T, V) with the same kernel-batch/head/token layout as ``q``),
        so ``out``
        ...
        if get_spmd_backend() == "spmd_types" and spmd.is_type_checking():
            ...
            # Aux outputs are (B, H, T) = q minus the trailing head dim.
    ...
    def forward(
        ...
        **kwargs,
        ...
        q_BHTK = q_THK.transpose(0, 1).unsqueeze(0)
        k_BHTK = k_THK.transpose(0, 1).unsqueeze(0)
        v_BHTV = v_THV.transpose(0, 1).unsqueeze(0)
        ...
        with maybe_regional_inductor(FlexAttention.inductor_configs):
            out_BHTV, aux = FlexAttention.compiled_flex_attn(
                q_BHTK,
                k_BHTK,
                v_BHTV,
                score_mod=score_mod,
        ...
        out_THV = out_BHTV.squeeze(0).transpose(0, 1)
        if out_transform is None:
            return out_THV
