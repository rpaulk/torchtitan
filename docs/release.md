...
- 1-GPU Fake-PG and 8-GPU Real-PG feature and model integration tests;
...
The workflow runs four 8-GPU jobs: two on A10G and two on H100. All integration
suites use real process groups:
...
versions. RL, TorchFT, and the Transformers modeling backend are intentionally
not included.
...
- the Step 4B **Validate a TestPyPI Release Candidate on GPUs** workflow is
  ...
  A10G`, `Validate core H100 tests`, and `Validate GraphTrainer H100 tests`;
...
1. **Pre-branch CI on `main`:** lint, CPU/GPU unit tests, Fake-PG and Real-PG
   integration tests, H100 tests, and the latest available CI for projects
   under `torchtitan/experiments/`.
...
4. **TestPyPI GPU validation:** four 8-GPU jobs, two on A10G and two on H100,
   install the staged RC and pinned GPU packages. All integration suites use
   real process groups and cover GPU unit tests, feature/model integration
   tests, loss and gradient-norm goldens, Flux, GraphTrainer, and the dedicated
   H100 suites.
...
- **GraphTrainer AutoParallel:** standard and H100 suites and numerics.
- **Experimental projects:** RL, TorchFT, and Transformers modeling backend.
